...
const Teeva = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for 'teeva' category
        const teevaProducts = allProducts.filter(product => {
          if (!product.category) return false;
          
          // Handle case where category is an object
          if (typeof product.category === 'object') {
            return product.category.name && product.category.name.toLowerCase() === 'teeva';
          }
          
          // Handle case where category is a string
          return product.category.toLowerCase() === 'teeva';
        });

        setProducts(teevaProducts);
      } catch (err) {
  ...
  const inStockCount = filteredAndSortedProducts.length;
  ...
  const renderWithLineBreaks = (text) => {
    ...
    return lines.map((line, idx) => (
      <div key={`line-${idx}`} className="description-line">
        {line}
                ...
                <div className="product-details">
                  ...
                  {activeView === 'list' && (
                    <p className="product-description">
                      {renderWithLineBreaks(product.description)}
                    </p>
                  )}
